...
<div class="accordion" id="pendingAllowancesAccordion">
    @foreach($datas as $batchKey => $group)
        ...
        <div class="card border shadow-sm mb-3 rounded overflow-hidden">
            ...
            <div class="card-header bg-white p-3 d-flex flex-wrap align-items-center justify-content-between gap-3" id="heading_{{ $cleanCollapseId }}">
                ...
                <div class="d-flex align-items-center gap-4">
                    ...
                    <div class="d-flex gap-1">
                        <button type="button" class="btn btn-sm btn-success px-3" onclick="confirmAction('approve-form-{{ $sampleRecord->id }}', 'approve this allowance batch')">
                            <i class="fas fa-check-circle me-1"></i> Approve Batch
                        </button>
                        <button type="button" class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#rejectModal{{ $sampleRecord->id }}">
            ...
            <!-- Forms Supporting Instant Bulk Action Executions -->
            <form id="approve-form-{{ $sampleRecord->id }}" action="{{ route('approveweeklyallowance', $encryptedId) }}" method="POST" class="d-none">@csrf</form>

            <!-- Expanded Accordion Matrix Rows -->
            <div id="{{ $cleanCollapseId }}" class="collapse" aria-labelledby="heading_{{ $cleanCollapseId }}" data-bs-parent="#pendingAllowancesAccordion">
                        ...
                        <table class="table table-hover bg-white mb-0 text-nowrap align-middle">
                            <thead class="bg-light text-secondary small text-uppercase">
                                <tr>
                                    ...
                                    <th class="border-0">Assigned Reference</th>
                                    <th class="text-end border-0">Base Amount</th>
                                    <th class="text-end border-0 pe-3">Comment Note</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-end font-weight-bold">{{ number_format($item->AllowanceAmount, 2) }}</td>
                                    <td class="text-end text-muted small pe-3 italic">{{ $item->AllowanceComment ?? '-' }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
        ...
        <!-- REJECT MODAL -->
        <div class="modal fade" id="rejectModal{{ $sampleRecord->id }}" data-bs-backdrop="static" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog">
                <form action="{{ route('rejectweeklyallowance', $encryptedId) }}" method="POST">
                    @csrf
                    <div class="modal-content shadow-lg">
                        <div class="modal-header bg-danger text-white">
                            <h5 class="modal-title"><i class="fas fa-times-circle me-2"></i> Reject Allowance Batch</h5>
                            <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <p class="small text-secondary">State the revision request details or compliance flags. This logs an operational error flag for tracking.</p>
                            <div class="mb-3">
                                <label class="form-label font-weight-bold small text-dark">Reason / Rejection Comments <span class="text-danger">*</span></label>
                                <textarea name="HrManagerComnt" rows="3" class="form-control" placeholder="Enter details regarding structural adjustment needs..." required></textarea>
                            </div>
                        </div>
