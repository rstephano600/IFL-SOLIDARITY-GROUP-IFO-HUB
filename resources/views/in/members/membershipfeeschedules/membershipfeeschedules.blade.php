...
@section('title', 'Membership Fee Schedules')
@section('page-title', 'Membership Fee Schedules Management')

@section('content')
<div class="arbif-page-header">
    <h3><div class="page-icon"><i class="fas fa-file-invoice-dollar"></i></div>Membership Fee Schedules</h3>
    <button class="arbif-btn-submit" data-bs-toggle="modal" data-bs-target="#addFeeScheduleModal">
        <i class="fas fa-plus-circle me-1"></i> Add Fee Schedule
    </button>
</div>
...
<div class="arbif-card">
    <div class="arbif-card-body">
        <div class="arbif-table-wrap">
            <table class="arbif-table" id="membershipFeeSchedulesTable">
                <thead>
                    <tr>
                ...
                <tbody>
                    @forelse($feeSchedules as $index => $item)
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        ...
                        <td>
                            <a href="{{ route('viewmembershipfeeschedules', [encrypt($item->id)]) }}" class="arbif-btn-edit">
                                <i class="fas fa-eye"></i> View
                            </a>
                            <a href="{{ route('editmembershipfeeschedules', [encrypt($item->id)]) }}" class="arbif-btn-edit">
                                <i class="fas fa-pencil"></i> Edit
                            </a>
                            <a onclick="confirmDelete()" href="{{ route('destroymembershipfeeschedules', [encrypt($item->id)]) }}" class="arbif-btn-delete">
                    ...
                    <tr>
                        <td colspan="8" class="arbif-table-empty">
                            ...
                            No active membership fee schedules exist in the record.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
        ...
        <div class="arbif-pagination">
            <span class="arbif-pagination-info" data-table-info="membershipFeeSchedulesTable"></span>
            <div class="arbif-pagination-pages" data-table-pages="membershipFeeSchedulesTable"></div>
        </div>
    </div>
</div>

<!-- Fee Schedule Creation Modal -->
<div class="modal fade arbif-modal" id="addFeeScheduleModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <div class="modal-icon"><i class="fas fa-file-invoice-dollar"></i></div>
                <h5 class="modal-title">Create Membership Fee Schedule</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            
            <div class="modal-body">
                <form method="POST" id="dataFormFill" action="{{ route('storemembershipfeeschedules') }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-md-12">
                            ...
                            <textarea id="Description" name="Description" class="form-control" rows="2" placeholder="Brief details about this membership fee schedule...">{{ old('Description') }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="modal-footer shadow-sm bg-light rounded-bottom p-3" style="margin-top: 30px;">
                        ...
                        <button onclick="confirmSubmit()" type="submit" class="arbif-btn-submit">
                            <i class="bi bi-check2"></i> <span id="submitBtnText">Commit Fee Schedule</span>
                        </button>
                    </div>
                </form>
            </div>
